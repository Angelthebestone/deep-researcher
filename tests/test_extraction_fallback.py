...
class ExtractionFallbackTest(unittest.IsolatedAsyncioTestCase):
    async def test_local_fallback_extracts_mentions_when_gemini_times_out(self) -> None:
        adapter = _TimeoutingGeminiAdapter()
        service = ExtractionService(adapter=adapter, retry_attempts=1)

        mentions = await service.extract(
            "doc-1",
            "text",
            "file:///tmp/doc-1.txt",
