...
- **State Machine Isolation**: Investigation orchestration MUST use LangGraph. Do not manage research state manually in basic loops or recursions.
...
- **Decoupled Nodes**: LangGraph nodes in `pipeline/nodes.py` must be thin wrappers. Move LLM logic, prompting, and parsing to reusable services (e.g., `PlanningService`, `SynthesizerService`).
...
- **Canonical Research Identity**: Build one canonical research request before executing LangGraph. Once `document_id`, `idempotency_key`, `target_technology`, `breadth`, and `depth` are fixed, prompt refinement may only improve `query`, not mutate research identity.
- **Planner Uses Refined Brief**: `planificador_node` must consume the refined `query` as the research brief; do not plan off the raw target when a refined prompt already exists.
...
- **Split SSE Reports**: Do not overload `report` with different shapes. Use `report_markdown` for LangGraph research output and `report_artifact` for persisted document reports.
...
- **In-Node Prompting**: Do not hardcode prompts or LLM configurations inside `pipeline/nodes.py`.
...
- **Direct Adapter instantiation in Nodes**: Nodes should not instantiate `GeminiAdapter`. They should receive a service that owns the adapter.
